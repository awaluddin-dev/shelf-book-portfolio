...
export default function DockNavigation({
  ...
  // Full SVG Path for the Top-Right L-Shape (180x240) - 4 rows tall
  const lShapePathFull = "M 20 0 L 160 0 A 20 20 0 0 1 180 20 L 180 220 A 20 20 0 0 1 160 240 L 140 240 A 20 20 0 0 1 120 220 L 120 80 A 20 20 0 0 0 100 60 L 20 60 A 20 20 0 0 1 0 40 L 0 20 A 20 20 0 0 1 20 0 Z";
  
  // Short SVG Path when Back To Top is hidden (180x180) - 3 rows tall
  const lShapePathShort = "M 20 0 L 160 0 A 20 20 0 0 1 180 20 L 180 160 A 20 20 0 0 1 160 180 L 140 180 A 20 20 0 0 1 120 160 L 120 80 A 20 20 0 0 0 100 60 L 20 60 A 20 20 0 0 1 0 40 L 0 20 A 20 20 0 0 1 20 0 Z";
  
  // Square SVG Path when menu is closed (60x60 at top-right corner)
  const lShapePathSquare = "M 140 0 L 160 0 A 20 20 0 0 1 180 20 L 180 40 A 20 20 0 0 1 160 60 L 140 60 A 20 20 0 0 1 120 40 L 120 20 A 20 20 0 0 0 120 20 L 120 20 A 20 20 0 0 1 120 20 L 120 20 A 20 20 0 0 1 140 0 Z";

  const activePath = !isOpen ? lShapePathSquare : (showBackToTop ? lShapePathFull : lShapePathShort);
  ...
  return (
    <motion.div
      ...
      className="fixed top-6 right-6 z-50 w-[180px] h-[240px] pointer-events-none group"
    >
      {/* Background and Border */}
      <div className="absolute inset-0 z-0">
        <svg width="180" height="240" viewBox="0 0 180 240" className="absolute inset-0 overflow-visible pointer-events-none">
          <defs>
            ...
            <linearGradient id="l-glow-grad" gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="180" y2="240">
              ...
              <animateTransform attributeName="gradientTransform" type="rotate" from="0 90 120" to="360 90 120" dur="4s" repeatCount="indefinite"/>
            </linearGradient>
          </defs>
          ...
          <motion.path 
            ...
            animate={{ d: activePath }} 
            transition={{ type: "spring", stiffness: 350, damping: 25 }}
            fill="none" stroke="url(#l-glow-grad)" strokeWidth="3" className="pointer-events-none" 
          />
          ...
          <motion.path 
            ...
            animate={{ d: activePath }} 
            transition={{ type: "spring", stiffness: 350, damping: 25 }}
            fill="none" stroke={isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)"} strokeWidth="1.5" className="pointer-events-none" 
          />
        </svg>
      ...
      <div className="absolute inset-0 grid grid-cols-3 grid-rows-4 z-10 pointer-events-none">
        
        {/* Top Left: Stack & Insights */}
        <AnimatePresence>
          {isOpen && (
            <motion.div
            ...
            >
              <button
                ...
                aria-label="Stack & Insights"
              >
                {activeSection === "proficiency" && (
                  <motion.div layoutId="activeDockButton" className="absolute inset-0 bg-neu-secondary/80 dark:bg-neu-secondary/60 rounded-xl border border-neu-accent/30" transition={{ type: "spring", stiffness: 350, damping: 25 }} />
                )}
                <div className={cn("relative z-10 transition-transform duration-300 group-hover:scale-110", activeSection === "proficiency" ? "text-neu-accent" : "text-neu-text-muted group-hover:text-neu-accent")}>
                  <Cpu size={18} />
                </div>
                {/* Tooltip BOTTOM */}
                <AnimatePresence>
          ...
          {isOpen && (
            ...
            >
              ...
              >
                ...
                <div className={cn("relative z-10 transition-transform duration-300 group-hover:scale-110", activeSection === "experience" ? "text-neu-accent" : "text-neu-text-muted group-hover:text-neu-accent")}>
                  <Briefcase size={18} />
                </div>
                {/* Tooltip BOTTOM */}
                <AnimatePresence>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="col-start-3 row-start-2 flex items-center justify-center pointer-events-auto"
            >
              <button
              ...
              >
                ...
                <div className={cn("relative z-10 transition-transform duration-300 group-hover:scale-110", activeSection === "endorse" ? "text-neu-accent" : "text-neu-text-muted group-hover:text-neu-accent")}>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="col-start-3 row-start-3 flex items-center justify-center pointer-events-auto"
            >
              <button
              ...
              >
                <div className={cn("relative z-10 transition-transform duration-300 group-hover:rotate-12 group-hover:scale-110", "text-neu-text-muted group-hover:colorful")}>
                  <Palette size={18} />
                </div>
                {/* Tooltip LEFT */}
                <AnimatePresence>
        ...
        <AnimatePresence>
          {isOpen && showBackToTop && (
            <motion.div
              ...
              className="col-start-3 row-start-4 flex items-center justify-center pointer-events-auto"
            >
              <button
                onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
                onMouseEnter={() => setHoveredDockId("scroll-top")}
                onMouseLeave={() => setHoveredDockId(null)}
                className="group relative flex items-center justify-center w-12 h-12 rounded-xl hover:bg-neu-secondary/50 dark:bg-neu-secondary/30 active:scale-90 transition-all cursor-pointer"
