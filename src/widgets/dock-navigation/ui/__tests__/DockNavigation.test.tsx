...
describe("DockNavigation", () => {
  ...
  it("shows tooltips on hover", () => {
    ...
    expect(screen.getByText("Projects", { selector: '.absolute' })).toBeInTheDocument();
  ...
  it("shows active indicator for all active sections", () => {
    const { rerender } = render(<DockNavigation isDark={true} showBackToTop={true} activeSection="projects" openPlayground={mockOpenPlayground} />);
    expect(screen.getByLabelText("Projects").querySelector('.border-neu-accent\\/30')).toBeInTheDocument();

    rerender(<DockNavigation isDark={true} showBackToTop={true} activeSection="proficiency" openPlayground={mockOpenPlayground} />);
    expect(screen.getByLabelText("Proficiency").querySelector('.border-neu-accent\\/30')).toBeInTheDocument();

    rerender(<DockNavigation isDark={true} showBackToTop={true} activeSection="experience" openPlayground={mockOpenPlayground} />);
    expect(screen.getByLabelText("Experience").querySelector('.border-neu-accent\\/30')).toBeInTheDocument();

    rerender(<DockNavigation isDark={true} showBackToTop={true} activeSection="endorse" openPlayground={mockOpenPlayground} />);
    expect(screen.getByLabelText("Endorse").querySelector('.border-neu-accent\\/30')).toBeInTheDocument();
  });
  ...
  it("shows tooltips for all buttons", () => {
    ...
    buttons.forEach(({ label, tooltip }) => {
      ...
      expect(screen.getByText(tooltip, { selector: '.absolute' })).toBeInTheDocument();
      fireEvent.mouseLeave(screen.getByLabelText(label));
      expect(screen.queryByText(tooltip, { selector: '.absolute' })).not.toBeInTheDocument();
    });
  });
});
