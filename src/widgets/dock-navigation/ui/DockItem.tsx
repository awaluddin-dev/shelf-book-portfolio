      ...
      >
        ...
        <div
          className={cn(
            ...
            hoverAnimation,
            isActive ? "text-neu-accent" : cn("text-neu-text-muted", hoverColor)
          )}
        >
          {icon}
        </div>
          ...
          {hoveredDockId === id && (
            <motion.div
              initial={{ 
                opacity: 0, 
                scale: 0.8,
                ...(tooltipPos === "BOTTOM" ? { y: -10, x: "-50%" } : { x: 10, y: "-50%" })
              }}
              animate={{ 
                opacity: 1, 
                scale: 1,
                ...(tooltipPos === "BOTTOM" ? { y: 0, x: "-50%" } : { x: 0, y: "-50%" })
              }}
              exit={{ 
                opacity: 0, 
                scale: 0.8,
                ...(tooltipPos === "BOTTOM" ? { y: -6, x: "-50%" } : { x: 6, y: "-50%" })
              }}
              transition={{
                type: "spring",
                stiffness: 450,
