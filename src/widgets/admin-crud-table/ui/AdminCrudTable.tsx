...
export function AdminCrudTable<T extends { id?: string }>({
  ...
  onBeforeSave,
  customActions,
}: Readonly<AdminCrudTableProps<T>>) {
  ...
  const handleSave = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      setToastMessage({
        message: `Successfully ${editingItem ? "updated" : "added"} ${itemName}`,
        type: "success",
      });
      setShowModal(false);
      setEditingItem(null);
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="glass-card-inset text-xs font-mono uppercase text-neu-text-muted">
                  <tr>
                    {columns.map((col, idx) => (
                      <th key={idx as number} className="px-6 py-4 font-bold">
                        {col.header}
                      </th>
                    ))}
                  ...
                  {items.length === 0 ? (
                    <tr>
                      <td
                        ...
                        className="px-6 py-8 text-center text-neu-text-muted font-mono"
                      >
                    ...
                    paginatedItems.map((item: T, idx: number) => (
                      ...
                      >
                        {columns.map((col, cIdx) => (
                          <td key={cIdx as number} className="px-6 py-4">
                            {col.render(item)}
                          </td>
                        ))}
                        <td className="px-6 py-4 flex justify-end gap-2">
                          ...
                          <button
                            ...
                            className="p-2 rounded-xl glass-card text-neu-accent hover:scale-105 active:scale-95 transition-all"
                            title="Edit"
                          >
                            <Edit size={16} />
                          </button>
