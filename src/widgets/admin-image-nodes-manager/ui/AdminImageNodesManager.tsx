...
interface AdminImageNodesManagerProps {
  title: string;
  apiEndpoint: string;
...
export function AdminImageNodesManager({
  title,
  apiEndpoint,
  imagePlaceholder,
  imageAlt,
}: AdminImageNodesManagerProps) {
  return (
        ...
        <>
          <div className="flex flex-col gap-4 mb-4">
            <div className="flex gap-4">
              ...
              <input placeholder={imagePlaceholder} value={formData.imageUrl || ''} onChange={e => setFormData({...formData, imageUrl: e.target.value})} className="w-full px-3 py-2 rounded-lg glass-card-inset text-sm outline-none" />
            </div>
          ...
          <div className="flex gap-4 items-start w-full">
            ...
            <div className="flex-1 flex flex-col gap-2">
              <div className="w-full aspect-video rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 overflow-hidden flex items-center justify-center">
                {item.imageUrl ? (
                  <img src={item.imageUrl} alt={imageAlt} className="w-full h-full object-contain" />
                ) : (
