...
const ProjectDetailsPage = ({ selectedProject }: { selectedProject: any }) => (
  <div className="flex flex-col h-full space-y-8">
    <div>
      <h2 className="text-3xl font-display font-black tracking-tight mb-2 text-zinc-900 dark:text-white">
        {selectedProject.title}
      </h2>
      <p className="text-sm italic font-mono text-zinc-600 dark:text-zinc-400">
    ...
    <div className="flex flex-wrap items-center gap-3">
      {selectedProject.demoUrl && (
        <a
          ...
          className="inline-flex items-center gap-2 px-4 py-2 bg-neu-accent hover:bg-neu-accent/90 text-white rounded-lg font-mono text-[10px] font-bold shadow-md transition-transform active:scale-95"
        >
      ...
      {selectedProject.github && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-4 py-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 rounded-lg font-mono text-[10px] font-bold shadow-sm border border-zinc-300 dark:border-zinc-700 transition-transform active:scale-95"
        >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {selectedProject.stats.map((stat: any, idx: number) => (
            <div
              key={idx as number}
              className="p-3 rounded-xl bg-white dark:bg-zinc-900 shadow-sm border border-zinc-200 dark:border-zinc-800 flex flex-col justify-between items-start text-left"
            >
              <span className="text-lg font-bold font-display text-zinc-900 dark:text-white tracking-tight">
                {stat.value}
              </span>
              <span className="text-[9px] font-mono text-zinc-500 dark:text-zinc-400 mt-1">
                {stat.label}
              </span>
            </div>
          ))}
...
const ProjectTechStackPage = ({
  selectedProject,
  getTechIconAndColor,
}: {
  selectedProject: any;
  getTechIconAndColor: any;
  activeProjects: any[];
}) => (
  <div className="flex flex-col h-full space-y-8">
    {(selectedProject.reasonToBuild || selectedProject.problemSolved) && (
      <div className="flex flex-col gap-6">
        {selectedProject.reasonToBuild && (
          <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800">
            <h4 className="text-[10px] font-mono font-bold text-neu-accent uppercase tracking-wider mb-3 flex items-center gap-2">
              <Lightbulb size={14} className="text-neu-accent" /> Why I Built
              This
            </h4>
        ...
        {selectedProject.problemSolved && (
          <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800">
            <h4 className="text-[10px] font-mono font-bold text-neu-accent uppercase tracking-wider mb-3 flex items-center gap-2">
              <Target size={14} className="text-neu-accent" /> Business Problem
              Solved
            </h4>
            <p className="text-zinc-600 dark:text-zinc-400 text-xs leading-relaxed">
              {selectedProject.problemSolved}
      ...
      <div className="flex flex-wrap gap-2">
        {(selectedProject.tags || []).map((tag: string) => {
          ...
          return (
            <div
              key={tag as string}
              className="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-[10px] font-mono font-medium shadow-sm"
            >
...
const ProjectMarkdownPage = ({
  ...
  <div className="w-full h-full flex flex-col">
    ...
    {content ? (
      <div
        className="flex-1 min-h-0 overflow-y-auto custom-scrollbar prose prose-slate prose-sm max-w-none font-sans
          ...
          components={{
            code({ className, children, ...props }: any) {
              ...
              if (!match)
                return (
                  <code
                    className="bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-pink-500 text-xs font-mono"
                    {...props}
                  >
                    {children}
                  </code>
      ...
      <p className="text-xs text-zinc-500 italic">
        No markdown specifications provided.
      </p>
    )}
  </div>
);
...
const ProjectErdPage = ({
  ...
  <div className="w-full h-full flex flex-col">
    <h4 className="text-[10px] font-mono font-bold text-neu-accent uppercase tracking-wider mb-6 flex items-center gap-2 border-b border-zinc-200 dark:border-zinc-800 pb-2 shrink-0">
      <Database size={14} /> Entity Relationship Diagram{" "}
      {total > 1 ? `(${index + 1}/${total})` : ""}
    </h4>
    <div className="flex-1 min-h-0 bg-white dark:bg-zinc-900/50 rounded-xl p-2 border border-zinc-200 dark:border-zinc-800 shadow-inner overflow-hidden flex flex-col relative gap-2">
      <div className="flex-1 w-full flex items-center justify-center overflow-hidden bg-zinc-50 dark:bg-[#0d1117] rounded-lg">
        ...
        {getRelatedProjects(selectedProject, activeProjects)
          ...
          .map((proj) => (
            ...
            >
              <div
                className={cn(
                  "w-10 h-12 rounded-sm flex-shrink-0 flex items-center justify-center relative shadow-sm",
              ...
              ></div>
              <div className="bg-white/5 border border-white/10 rounded-lg p-4 mb-4"></div>
              <div className="flex-1 min-w-0">
                <span className="text-[8px] font-mono text-neu-accent font-bold uppercase tracking-wider block">
                  {proj.category}
                </span>
                <h5 className="text-xs font-bold text-zinc-900 dark:text-white truncate mt-0.5 group-hover:text-neu-accent transition-colors">
                  {proj.title}
                </h5>
              </div>
            </button>
...
export default function ProjectModal({
  ...
  const onSelectProject = useCallback(
    (project: any) => setSelectedProject(project),
    [setSelectedProject],
  );
  ...
  const renderInteriorSection = (data: any) => {
    ...
    switch (type) {
      ...
      case "schema":
        return (
          <ProjectSchemaPage content={content} index={index} total={total} />
        );
      case "erd":
        ...
        <div className="w-full h-full flex flex-col relative bg-zinc-900 rounded-r-[2rem] rounded-l-md overflow-hidden shadow-2xl border-l-4 border-l-black/50 border-y border-r border-white/10">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center h-full p-8 text-center border-[6px] border-white/5 m-6 rounded-2xl backdrop-blur-[2px]">
            ...
            <div className="absolute bottom-6 left-0 right-0 flex flex-col items-center">
              <span className="text-[9px] font-mono text-white/30 uppercase tracking-[0.3em] mb-1">
                Author
              </span>
              <span className="text-[11px] font-mono text-white/80 font-bold uppercase tracking-[0.2em]">
                Awaluddin
              </span>
            </div>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center justify-center h-full p-10 text-center">
            <div className="mb-12 p-6 rounded-3xl border border-white/5 bg-white/5 backdrop-blur-sm shadow-xl">
              ...
              <p className="text-[11px] md:text-xs text-white/60 font-mono italic leading-relaxed max-w-[220px]">
                ...
                engineering. Every line of code is a brushstroke.&quot;
              </p>
            </div>
                ...
                {[...Array(28)].map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      height: "100%",
                      opacity: secureMathRandom() > 0.2 ? 1 : 0,
                    }}
                  ></div>
                ))}
              </div>
            </div>
            <span className="text-[7px] font-mono text-white/30 tracking-[0.4em]">
              ISBN 978-AWAL-PORTFOLIO
            </span>
          </div>
  ...
  const isSinglePage =
    spreads[currentPage]?.type === "front-cover" ||
    spreads[currentPage]?.type === "back-cover";
      ...
      >
        ...
        <motion.div
          ...
          className={cn(
            ...
            isSinglePage
              ? "max-w-[380px] md:max-w-[550px] aspect-[2/3] md:aspect-[1/1.4]"
              : "max-w-[380px] md:max-w-[1100px]",
          )}
