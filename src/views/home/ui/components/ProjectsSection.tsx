...
import { Search, Filter, BookOpen, ChevronLeft, ChevronRight, X } from "lucide-react";
import BookItem from "@/entities/project/ui/BookItem";

import { Code2 } from "lucide-react";
export function ProjectsSection({ pd, triggerToast, activeProjects, isLoading, isDark, focusedProject, setFocusedProject, setSelectedProject, getTagProjectCount }: any) {
    const [searchQuery, setSearchQuery] = useState("");
    const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
    const [sortBy, setSortBy] = useState<"newest" | "oldest" | "alphabetical">("newest");
    const [isFilterModalOpen, setIsFilterModalOpen] = useState(false);
    
    const categories: string[] = useMemo(() => Array.from(new Set((activeProjects || []).map((p: any) => p.category))), [activeProjects]);
