...
export default function AdminPlayground({
  ...
  useEffect(() => {
    const isDifferent =
      JSON.stringify(draftConfig) !== JSON.stringify(themeConfig);
    if (!isDifferent) {
      return;
    }
  ...
  useEffect(() => {
    ...
    root.style.setProperty(
      "--shadow-neu",
      `8px 8px ${blur}px rgba(0,0,0,${sOpacity}), -8px -8px ${blur}px ${lightShadowHex}`,
    );
    root.style.setProperty(
      "--shadow-neu-inset",
      `inset 6px 6px ${blur - 4}px rgba(0,0,0,${sOpacity}), inset -6px -6px ${blur - 4}px ${lightShadowHex}`,
    );
    root.style.setProperty(
      "--shadow-neu-sm",
      `4px 4px ${blur / 2}px rgba(0,0,0,${sOpacity}), -4px -4px ${blur / 2}px ${lightShadowHex}`,
    );
    root.style.setProperty(
      "--shadow-neu-modal",
      `16px 16px ${blur * 2}px rgba(0,0,0,${sOpacity * 1.5})`,
    );
  ...
  return (
    <div
      className={`fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 transition-all duration-500 ${isPreviewMode ? "pointer-events-none" : ""}`}
    >
      <AnimatePresence>
        {!isPreviewMode && (
          <motion.div
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-5 space-y-6">
                  <div className="p-6 rounded-3xl glass-card border border-white/5 space-y-6">
                    <h3 className="font-bold border-b border-black/10 dark:border-white/10 pb-3">
                      Colors
                    </h3>
                      ...
                      {[
                        ...
                        <div key={color.key} className="flex flex-col gap-2">
                          <label className="text-xs font-mono font-bold text-neu-text-muted flex justify-between">
                            {color.label}{" "}
                            <span>{(draftConfig as any)[color.key]}</span>
                          </label>
                          <div className="flex items-center gap-3">
                            <input
                              ...
                              onChange={(e) =>
                                updateConfig(color.key, e.target.value)
                              }
                  ...
                  <div className="p-6 rounded-3xl glass-card border border-white/5 space-y-6">
                    ...
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-xs font-mono font-bold text-neu-text-muted flex justify-between">
                          Shadow Intensity{" "}
                          <span>{draftConfig.shadowOpacity.toFixed(2)}</span>
                        </label>
                        <input
                          type="range"
                          min="0.01"
                ...
                <div className="lg:col-span-7 sticky top-8 space-y-8">
                  ...
                  <div className="p-8 rounded-3xl glass-card space-y-6">
                    ...
                    <div className="flex gap-4">
                      <button
                        type="button"
                        className="flex-1 py-3 px-4 rounded-xl shadow-neu bg-neu-bg text-neu-accent font-bold hover:shadow-neu-sm active:shadow-neu-inset transition-all"
                      >
                        Neumorphic Button
                      </button>
                      <button
                        type="button"
                        className="flex-1 py-3 px-4 rounded-xl shadow-neu bg-neu-accent text-white font-bold hover:opacity-90 active:scale-95 transition-all"
                      >
                        Accent Button
                      </button>
                    </div>
                    ...
                    <div className="space-y-2">
                      <label
                        htmlFor="id-sample-input-playground"
                        className="text-xs font-mono opacity-70"
                      >
                        Sample Input
                      </label>
                      <input
                        id="id-sample-input-playground"
                        type="text"
