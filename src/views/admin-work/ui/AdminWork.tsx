    ...
    <>
      <AdminCrudTable
        ...
        onBeforeSave={(formData) => ({
          ...formData,
          bullets: typeof formData.bullets === 'string' ? formData.bullets.split('\n').map((b: string) => b.trim()).filter(Boolean) : formData.bullets
        })}
        customActions={(item) => (
          <button onClick={() => setViewingWork(item)} className="p-2 rounded-xl glass-card text-neu-text-muted hover:text-neu-text hover:scale-105 active:scale-95 transition-all" title="View Detail">
            <Eye size={16} />
          </button>
        )}
        columns={[
          {
            ...
            render: (item: any) => (
              <>
                ...
                <div className="text-xs text-neu-text-muted mt-1">{item.company}</div>
              </>
            )
          },
          {
            ...
            render: (item: any) => (
              <>
                <div className="text-xs font-bold text-neu-accent">{item.years}</div>
                <div className="text-xs text-neu-text-muted">{item.duration}</div>
              </>
            )
          }
        ]}
          ...
          <>
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                    ...
                    <input required value={formData.company || ''} onChange={e => setFormData({...formData, company: e.target.value})} className="w-full px-4 py-2.5 rounded-xl glass-card-inset text-sm font-medium border border-white/5 focus:border-neu-accent outline-none" placeholder="e.g. Acme Corp" />
                </div>
                <div className="space-y-1">
                    <label className="text-xs font-mono text-neu-text-muted">Role</label>
                    <input required value={formData.role || ''} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full px-4 py-2.5 rounded-xl glass-card-inset text-sm font-medium border border-white/5 focus:border-neu-accent outline-none" placeholder="e.g. Senior Engineer" />
                </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
            ...
            <div className="space-y-1">
                ...
                <textarea required value={Array.isArray(formData.bullets) ? formData.bullets.join('\n') : (formData.bullets || '')} onChange={e => setFormData({...formData, bullets: e.target.value})} rows={4} className="w-full px-4 py-2.5 rounded-xl glass-card-inset text-sm font-medium border border-white/5 focus:border-neu-accent outline-none resize-none" placeholder="Led a team of 5...&#10;Increased performance by 20%..." />
            </div>
          </>
        )}
      />
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-neu-bg rounded-3xl shadow-neu-modal w-full max-w-2xl p-8 relative border border-white/5 max-h-[85vh] overflow-y-auto hide-scrollbar">
            ...
            <div className="mt-8 flex justify-end gap-3">
              <button onClick={() => setViewingWork(null)} className="px-6 py-2.5 rounded-xl font-bold text-white bg-neu-accent hover:shadow-neu-sm transition-colors text-sm">
                Close
              </button>
            </div>
          </div>
