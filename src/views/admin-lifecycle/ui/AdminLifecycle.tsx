        ...
        <>
          <div className="flex gap-4">
            <select
              ...
              className="w-1/3 px-3 py-2 rounded-lg glass-card-inset text-sm outline-none border border-transparent focus:border-neu-accent"
            >
            ...
            <input
              ...
              className="w-1/3 px-3 py-2 rounded-lg glass-card-inset text-sm outline-none"
            />
            <input
              ...
              className="w-20 px-3 py-2 rounded-lg glass-card-inset text-sm outline-none"
            />
          </div>
          <input
            placeholder="Title (e.g. Initial Architecture Design)"
            value={formData.title}
            onChange={(e) =>
              setFormData({ ...formData, title: e.target.value })
            }
            className="w-full px-3 py-2 rounded-lg glass-card-inset text-sm outline-none"
          />
          <textarea
            ...
            className="w-full px-3 py-2 rounded-lg glass-card-inset text-sm outline-none min-h-[80px]"
          />
          <input
            placeholder="Evidence URL (e.g. https://link-to-pdf.com) - Optional"
            value={formData.evidentUrl || ""}
            onChange={(e) =>
              setFormData({ ...formData, evidentUrl: e.target.value })
            }
            className="w-full px-3 py-2 rounded-lg glass-card-inset text-sm outline-none"
          />
        </>
          ...
          <div className="flex gap-4 items-start">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-mono bg-black/5 dark:bg-white/5 px-2 py-0.5 rounded text-neu-text-muted">
                  {item.stage}
                </span>
                <span className="text-[10px] font-mono font-bold text-neu-accent">
                  {item.date}
                </span>
              </div>
              <h3 className="font-bold text-lg text-neu-text mb-1">
                {item.title}
              </h3>
              <p className="font-medium text-neu-text-muted text-sm">
                {item.description}
              </p>

              {item.evidentUrl && (
                <a
                  href={item.evidentUrl}
                  target="_blank"
                  rel="noreferrer"
