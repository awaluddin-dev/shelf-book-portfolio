...
describe('useChat', () => {
  ...
  it('should handle API error', async () => {
    ...
    expect(result.current.error).toBe('Request failed: 500 Internal Server Error');
  });
  ...
  it('should handle stream error data format', async () => {
    const mockRead = jest.fn()
      .mockResolvedValueOnce({ done: false, value: new TextEncoder().encode('data: [ERROR] Something went wrong\n\n') })
      .mockResolvedValueOnce({ done: true, value: undefined });
