...
describe("AdminCrudTable", () => {
  ...
  const mockCustomActions = jest.fn((item) => (
    <button data-testid={`custom-${item.id}`}>Custom</button>
  ));
  ...
  it("handles edit and save successfully", async () => {
    ...
    render(
      <AdminCrudTable
        {...defaultProps}
        onBeforeSave={(data) => ({ ...data, modified: true })}
      />,
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Successfully updated TestItem"),
      ).toBeInTheDocument();
    });
  ...
  it("handles delete successfully and cancellation", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Successfully deleted TestItem"),
      ).toBeInTheDocument();
    });
  });
